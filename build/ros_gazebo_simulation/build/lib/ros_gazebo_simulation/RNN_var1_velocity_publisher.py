...
def sighandler(signal, frame):
    ...
    np.savetxt("RNN_GRU_var1_output_joint.csv", combined_array, delimiter=",", header="Real_linear, Prediction_linear, Real_angular, Prediction_angular,  joint_state_velocity_x, joint_state_velocity_y")
...
class LidarSubscriber(Node):
    ...
    def __init__(self):
        ...
        self.linear_vel_model = models.load_model('/home/joanna/ros2_gz_sim/neural_networks/RNN_GRU_model_linear_velocity_wariant_1.keras')
        self.angular_vel_model = models.load_model('/home/joanna/ros2_gz_sim/neural_networks/RNN_GRU_model_angular_velocity_wariant_1.keras')
        self.predicted_velocity = Twist()


    def timer_callback(self):
        self.present_lidar_data = self.lidar_data[self.i]
        self.processed_lidar_data = np.zeros(72)
        for j in range(36):
            self.processed_lidar_data[j] = self.old_lidar_data[j*10]
        index = 0
        for j in range(36 , 72):
            self.processed_lidar_data[j] = self.present_lidar_data[index*10]
            index+=1

        self.old_lidar_data = self.present_lidar_data

        self.processed_lidar_data = self.processed_lidar_data.reshape(1, 72, 1)
